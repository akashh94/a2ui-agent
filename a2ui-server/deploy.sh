...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --set-env-vars "AGENT_ENGINE_PROJECT=${AGENT_ENGINE_PROJECT},AGENT_ENGINE_LOCATION=${AGENT_ENGINE_LOCATION},AGENT_ENGINE_ID=${AGENT_ENGINE_ID},ALLOW_ORIGINS=${ALLOW_ORIGINS},CATALOG_URL=${CATALOG_URL}" \
  --min-instances 1 \
  --max-instances 1
